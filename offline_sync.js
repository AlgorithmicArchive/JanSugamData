...
async function main() {
  ...
  const files = fs
    ...
    .filter((f) => f.startsWith("jansugam_") && f.endsWith(".json"))
    .sort();

  if (files.length === 0) {
    console.log("No jansugam_*.json files found!");
    await sql.close();
    return;
  }
  ...
  for (const file of files) {
    let data;
    try {
      data = JSON.parse(fs.readFileSync(path.join(__dirname, file), "utf-8"));
    } catch (err) {
      console.error(`Failed to parse ${file}:`, err.message);
      continue;
    }

    if (data.initiated_data?.length > 0) {
      for (const [i, e] of data.initiated_data.entries()) {
        await processAndInsertInitiatedData(e, pool);
        if (i % 50 === 49) console.log(`     Initiated: ${i + 1} done`);
      }
    }
    ...
    initiatedMap = {};
    mapRes.recordset.forEach(
      (r) => (initiatedMap[r.appl_id] = { appl_ref_no: r.ReferenceNumber })
    );

    if (data.execution_data?.length > 0) {
      let cnt = 0;
      for (const e of data.execution_data) {
        await processAndInsertExecutionData(e, pool);
        if (++cnt % 100 === 0) console.log(`     Execution: ${cnt} done`);
      }
    }
